...
class Khabarwala_api extends CI_Controller {
   ...
   public function __construct() 
   {
    ...
    public function getRestaurents()
    {
        ...
        $restaurents = $this->db->query("SELECT * FROM tb_restaurant WHERE published=1")->result();
        $allRes = array();
        
        foreach ($restaurents as $res) {

            $menus = $this->db->query("SELECT * FROM tb_menu WHERE published=1 AND res_id=".$res->res_id)->result();
            $strMenu = '';
    ...
    public function getRestaurentDetails()
    {
        ...
        if(!empty($cuisines))
        {
        	foreach ($cuisines as $cuisine) {
	        	...
	        	$menus = $this->db->query("SELECT * FROM tb_menu WHERE published=1 AND res_id=".$res_id." AND cuisine_id=".$cuisine->cuisine_id)->result();
	...
	function add_cart()
	{
		...
		if($restaurantDetail->is_gst_applied=='y')
		{
			$res_gst = $restaurantDetail->gst_amount;
		}
		else{
	...
	function place_order()
	{
		...
		if($last_bill_no)
		{
			$sl_no = $last_bill_no->sl_no+1;
			$next_sl_no = str_pad($sl_no,4,'0',STR_PAD_LEFT);
			$order_gen_id = "KHB".$next_sl_no;
		}
		else
		{
			$next_sl_no = str_pad(1,4,'0',STR_PAD_LEFT);
			$order_gen_id = "KHB".$next_sl_no;	
		}
		/* next invoice number */	
		...
		if($cartCounts)
		{
			foreach($cartCounts as $cartCount)
			{
				...
				$resId = $cartCount->restaurant_id;
			}
		}

		// Get Order Details 
		$orderMenus = $this->db->query("SELECT tb_order_menu.*,tb_menu.menu_name FROM `tb_order_menu` INNER JOIN tb_menu ON tb_menu.menu_id=tb_order_menu.menu_id WHERE tb_order_menu.order_id=".$order_id)->result();
		$users = $this->db->query("SELECT * FROM tb_user WHERE user_id=".$user_id)->row();
		$restaurant = $this->db->query("SELECT * FROM tb_restaurant WHERE res_id=".$resId)->row();
		$orderDetails = $this->db->query("SELECT * FROM tb_order WHERE order_id=".$order_id)->result();
		...
		$cartCount = $this->db->query("select * from tb_cart where device_id='$deviceId'")->num_rows();
		header('Access-Control-Allow-Origin: *');
		echo json_encode(array('status' => 1,'cartCount'=>$cartCount , 'orderMenus' => $orderMenus, 'users' => $users,  'restaurant' => $restaurant, 'orderDetails' => $orderDetails));
	}	
	...
	public function checkAppVersion()
	{
		...
		if($version!='')
		{
			$count = $this->db->query("SELECT * FROM tb_app_cur_version WHERE version='".$version."'")->num_rows();
			if($count>0)
			{
				$temp = 1;
			}
	...
	// 	$restaurents = $this->db->query("SELECT * FROM tb_restaurant a INNER JOIN tb_menu b ON a.res_id=b.res_id  WHERE a.published=1 AND b.menu_name LIKE '%".$query."%'")->result();

	// 	//echo $this->db->last_query(); die;
	//     $allRes = array();

	//     //print_r($restaurents); die;
